...
{	
	// struct list_head weighted_rr_list_item
	// to denote the list item which will be inserted into the run queue of weighted_rr

	// structweighted_rr_rqweighted_rr
	// to specify the run queue for your weighted rr
...
static struct task_struct *pick_next_task_weighted_rr(struct rq *rq)
{
	...
	next->se.exec_start = rq->clock; // in struct sched_entity 		u64		exec_start;
...
static void task_tick_weighted_rr(struct rq *rq, struct task_struct *p,int queued)
{
	...
	if (p->task_time_slice && --p->task_time_slice) return;
